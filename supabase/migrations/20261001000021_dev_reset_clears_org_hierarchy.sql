-- ===========================================================================
-- Phase 13 / 0021 — dev_reset_all_data() also clears the organizational
-- hierarchy (states, districts, centres, workstations, the legacy
-- procurement_rates table).
--
-- 0019 kept these as "reference/configuration data" on the assumption a
-- development database's geography is worth keeping across a reset. In
-- practice the states/districts/centres already in a fresh project ARE the
-- thing being replaced — the retired prototype's reference data (a single
-- Tamil Nadu, two districts, five centres) — so a reset that keeps them
-- collides with a differently-shaped demo dataset (two states, four
-- districts, twelve centres) on the districts table's (state_id, name)
-- uniqueness.
--
-- Still kept: `crops` (a taxonomy, not a geography), `document_requirements`
-- (policy configuration) and `msp_rates` (rate configuration — the one
-- pre-existing row is scoped nationally, state_id/centre_id both null, so it
-- is never blocked by a district/centre being deleted).
--
-- `procurement_rates` (Phase 5) was already dropped by Phase 8, which
-- migrated its rows into `msp_rates` and replaced it as the one rate source
-- (see 20260801000017_payments_msp.sql) — nothing to delete here.
-- ===========================================================================
...
begin
  ...
  -- Organizational hierarchy. Order matters: workstations and centre_crops
  -- (RESTRICT / CASCADE respectively) before centres, centres before
  -- districts, districts before states.
  delete from public.queue_workstations           where true;
  delete from public.centre_crops                 where true;
  delete from public.procurement_centres          where true;
...
comment on function public.dev_reset_all_data() is
  'DEVELOPMENT ONLY. Deletes every account, all activity, and the organizational hierarchy (states/districts/centres); keeps the crop taxonomy, document requirements and MSP rate configuration. Refuses unless app.environment_flags.allow_data_reset is true; executable by service_role only.';
